...
public class AccountServiceImpl implements AccountServiceI {
		...
		if (!(custId == null || "null".equalsIgnoreCase(custId)) && !custId.isBlank()) {
			...
			if (!accountslist.isEmpty()) {
				...
				for (Account a : accountslist) {
					CustomerAccountsResponseDto custAcResp = new CustomerAccountsResponseDto(a.getAccountNumber(),
	...
	public NumOfRecordsResponseDto getAccountsByParameters(String customerId, Long accountNumber, Long mobileNumber,
		...
		List<Account> accountList = accountrepository.findByCustomersByParams(customerId, accountNumber, mobileNumber,
				panNumber, status, branchCode, pageable);
		if (!accountList.isEmpty()) {
			int count = accountList.size();
			NumOfRecordsResponseDto numDto = new NumOfRecordsResponseDto("Success", count,
					List.of("Account entities or DTOs should be there"));

			return numDto;
		} else {
			throw new CustomerNotFound(
					"Enter Any valid customerId: , accountNumber: , mobileNumber: , panNumber: , status: , branchCode:");
		}
